...
object ARIMA {
   ...
   */
  private def fitWithCSSBOBYQA(p: Int,
                               d: Int,
                               q: Int,
   ...
   */
  private[models] def hannanRissanenInit(p: Int, q: Int, y: Array[Double], includeIntercept: Boolean): Array[Double] = {
    val addToLag = 1
    ...
    val yTrunc = y.drop(m)
    val estimated = arTerms1.zip(Array.fill(yTrunc.length)(arModel.coefficients)).map {
      case (v, b) => v.zip(b).map { case (yi, bi) => yi * bi }.sum + arModel.c
    }
   ...
   * and Kandahar.
   */
  private def findBestARMAModel(diffedTs: Vector, maxP: Int, maxQ: Int, startWithIntercept: Boolean): ARIMAModel = {
    ...
    var nextParams = List((0, 0), (2, 2), (1, 0), (0, 1)).map {
      case (p, q) =>
    ...
    while (!done) {
      ...
      val modelsAndAICs =
        models.filter(m => m.isStationary && m.isInvertible).map(model => (model, model.approxAIC(diffedTs)))
      ...
      if (improvingModelsAndAICs.isEmpty) {
        ...
        // bounds.
        nextParams = surroundingParams.filter(
          params =>
            !pastParams.contains(params) &&
  ...
  private[models] def findRoots(coefficients: Array[Double]): Array[Complex] = {
    ...
    if (n > 1) {
      companionMatrix.setSubMatrix(
        MatrixUtils
          .createRealIdentityMatrix(n - 1)
          .getData(),
        0,
        1)
    }
...
class ARIMAModel(val p: Int, // AR order
