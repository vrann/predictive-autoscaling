...
object ARIMAX {
  ...
  def fitModel(p: Int,
    ...
    val params = fitWithCSSCGD(p, d, q, xregMaxLag, differentialTs, includeOriginalXreg, includeIntercept, initParams)
  ...
  private def fitWithCSSCGD(p: Int,
    ...
    val objFunction = new ObjectiveFunction(new MultivariateFunction() {
      def value(params: Array[Double]): Double = {
        new ARIMAXModel(p, d, q, xregMaxLag, params, includeOriginalXreg, includeIntercept)
    ...
    val (initialGuess, maxIter, maxEval, goal) =
      (new InitialGuess(initParams), new MaxIter(10000), new MaxEval(10000), GoalType.MAXIMIZE)
...
class ARIMAXModel(val p: Int,
                  ...
                  val includeIntercept: Boolean = true)
  ...
  def forecast(timeSeries: BreezeDenseVector[Double], xreg: Matrix[Double]): BreezeDenseVector[Double] = {
    ...
    iterateARMAX(
      differentialTsExtended,
      history,
      _ + _,
      goldStandard = differentialTsExtended,
      exogenousVar = differentialXreg)
    ...
    if (d != 0) {
      ...
      val forwardIntegrated =
        inverseDifferencesOfOrderD(BreezeDenseVector.vertcat(prevTermsForForwardInverse, forward(maxLag to -1)), d)
  ...
  private def iterateARMAX(ts: Vector,
    ...
    val xregMatrix = new BreezeDenseMatrix(
      rows = exogenousVar.flatten.length / exogenousVar.size,
      cols = exogenousVar.size,
      data = exogenousVar.flatten)
    val xregPredictors = AutoregressionX.assemblePredictors(
      Array.fill(xregMaxLag)(0.0) ++ ts.toArray,
      MatrixUtil.matToRowArrs(xregMatrix),
      0,
      xregMaxLag,
      includeOriginalXreg)
  ...
  def differenceXreg(xreg: Matrix[Double]): Array[Array[Double]] = {
    ...
    for (i <- 0 until xregColumns) {
      ...
      val diffXregCol =
        (new DenseVector(Array.fill(math.max(p, q))(0.0) ++ differencesOfOrderD(xregCol, d).toArray.drop(d))).toArray
  ...
  def removeTimeDependentEffects(ts: Vector, destTs: Vector): Vector = {
    ...
    val diffedExtended = new BreezeDenseVector[Double](Array.fill(maxLag)(interceptAmt) ++ diffed.toArray)
